...
- (BOOL)_dbOpen
{
    ...
    int result = sqlite3_open(_dataPath.UTF8String, &_db);
    if (result == SQLITE_OK) {
        ...
        if (_errorLogsEnabled) {
            NSLog(@"%s line:%d sqilte open failed (%d).", __FUNCTION__, __LINE__, result);
        }
        return NO;
    }
}
...
- (BOOL)_fileMoveAllToTrash
{
    ...
    BOOL suc = [[NSFileManager defaultManager] moveItemAtPath:_dataPath toPath:tmpPath error:NULL];
    if (suc) {
